...
def velocity_for_tiles(tile_ids: list[str]) -> dict[str, VelocityResult]:
    """Compute velocity only for the tiles needed by a paged list response.

    The full catalog calculation is intentionally kept for callers that need
    every tile, but the list endpoint should not pay that cost just to render
    the first page.
    """
    global _VELOCITY_CACHE
    now = monotonic()
    previous = _VELOCITY_CACHE[1] if _VELOCITY_CACHE and now - _VELOCITY_CACHE[0] < _VELOCITY_CACHE_TTL else {}
    missing = [tile_id for tile_id in tile_ids if tile_id not in previous]
    results = {**previous, **{tile_id: compute_velocity(tile_id) for tile_id in missing}}
    _VELOCITY_CACHE = (now, results)
    return {tile_id: results[tile_id] for tile_id in tile_ids if tile_id in results}
