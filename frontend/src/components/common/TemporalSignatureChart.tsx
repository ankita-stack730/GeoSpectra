...
export const TemporalSignatureChart: React.FC<TemporalSignatureChartProps> = ({ series, values, trend, className }) => {
  const data: ChartPoint[] = series
    ? series
      .filter((point) => Boolean(point?.date_pair?.before && point?.date_pair?.after))
      .map((point) => ({ ...point, label: point.date_pair.after }))
    : (values || [])
      .filter((velocity) => Number.isFinite(velocity))
      .map((velocity, index) => ({ velocity, label: `${index + 1}` }));
        ...
        <ResponsiveContainer width="100%" height="85%">
          <LineChart data={data} margin={{ top: 8, right: 8, left: -24, bottom: 0 }}>
            ...
            <Tooltip content={({ active, payload }) => {
              ...
              return (
                <div className="rounded-lg bg-space-950/90 border border-white/[0.1] p-2 font-mono text-[10px]">
                  {point.date_pair
                    ? <>{formatDate(point.date_pair.before)} → {formatDate(point.date_pair.after)}<br /></>
                    : <>Observation {point.label}<br /></>}
                  Velocity: {Number(point.velocity).toFixed(5)} / day
                  {point.source && <><br />Source: {point.source}</>}
                </div>
              );
            }} />
